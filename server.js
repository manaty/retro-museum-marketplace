...
import {collectionRoutes,processSelection,availableSelections,categoryIds} from './collection.js';
...
const processJob=(job,stores)=>job.kind==='selection'?processSelection(job,stores):job.kind==='quiz'?processQuiz(job,stores):processSubmission(job,stores);
...
export const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(role==='reviewer'){
   ...
   const result=await processJob(job,{inbox,outbox});send(res,result.pending?503:200,{status:result.status});return;
  }
  if(await handleCollection(req,res,path))return;
  if(await handleQuizzes(req,res,path))return;
  const quizFile={'/quizzes':'editor.html','/quiz-site-i18n.js':'site-i18n.js','/quiz-editor.js':'editor.js','/quiz-editor.css':'editor.css','/quiz-prompt.txt':'submit-prompt.txt'}[path];if(req.method==='GET'&&quizFile){res.writeHead(200,{'Content-Type':quizFile.endsWith('.html')?'text/html; charset=utf-8':quizFile.endsWith('.js')?'text/javascript; charset=utf-8':quizFile.endsWith('.css')?'text/css':'text/plain; charset=utf-8','Cache-Control':'no-cache','X-Content-Type-Options':'nosniff'});res.end(await readFile(new URL(import.meta.resolve('@manaty/game-quizz/public/'+quizFile))));return;}
  if(req.method==='GET'&&path==='/api/catalog'){
   const keys=await outbox.list('games/');const quizEntries=(await Promise.all((await outbox.list('quizzes/')).map(k=>outbox.get(k)))).filter(q=>q&&!q.withdrawn&&q.packageHash&&q.manifest).map(q=>({...q,sha256:q.packageHash,source:{url:'https://retro-museum.net/api/quizzes/content/'+q.sha256},fork:{url:'https://github.com/manaty/game-quizz'}}));const games=[...(await Promise.all(keys.map(k=>outbox.get(k)))),...quizEntries,...await availableSelections(outbox)].filter(g=>g).map(g=>({...g,catalogType:'reviewed',playUrl:playOrigin+'/g/'+encodeURIComponent(g.id)}));send(res,200,{schemaVersion:1,policyVersion:POLICY_VERSION,games:catalogEntries(firstParty,games)});return;
  }
  ...
  if(req.method==='POST'&&path==='/api/submissions'){
   ...
   const data=await body(req),decl=declarations(data.declarations),categories=categoryIds(data.categories),source=await resolveSubmission(data.repository);
   ...
   await inbox.put(`jobs/${id}.json`,{id,source,categories,declarations:decl,submittedAt:new Date().toISOString()},{create:true});
