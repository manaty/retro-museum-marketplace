...
import {validateQuiz,MAX_QUIZ_BYTES,difficultyCounts} from '@manaty/game-quizz/schema';import {BUILTIN_QUIZZES} from '@manaty/game-quizz/builtins';import {packageQuiz} from '@manaty/game-quizz/package-quiz';import {validatePackage} from '@manaty/retro-museum-sdk';
...
export async function processQuiz(job,{outbox,ai=reviewQuiz}){
 ...
 try{
  ...
  const progress=await outbox.get('quiz-review-progress/'+job.id+'.json');report.editorial=await ai(quiz,publication(job.publication),{progress:progress?.hash===job.hash?progress:null,maxBatches:4,saveProgress:p=>outbox.put('quiz-review-progress/'+job.id+'.json',{...p,hash:job.hash})});report.status=report.editorial.status;if(report.editorial.pending){report.pending=true;await outbox.put(path,report);return report;}
  if(report.status==='approved'){
   ...
   const base=JSON.parse(await readFile(new URL(import.meta.resolve('@manaty/game-quizz/package')),'utf8')),pack=packageQuiz(base,quiz.questions.length>100?{...quiz,questions:quiz.questions.slice(0,20)}:quiz);pack.licenseText+='\nQuestionnaire: '+quiz.author+' — CC BY 4.0. Sources: '+job.publication.sources.join(', ');
   ...
   await outbox.put('quizzes/'+job.hash+'.json',{id:'quiz-'+job.hash.slice(0,16),title:quiz.title,author:quiz.author,language:quiz.language,count:quiz.questions.length,levels:difficultyCounts(quiz.questions),sha256:job.hash,packageHash:quiz.questions.length<=100?hash:null,manifest:quiz.questions.length<=100?pack.manifest:null,reportId:job.id,license:'CC-BY-4.0',publishedAt:new Date().toISOString()});
  }
...
export function quizRoutes({inbox,outbox,queue}){
 return async(req,res,path)=>{
  ...
  if(req.method==='GET'&&path==='/api/quizzes'){
   ...
   json(res,200,{schemaVersion:1,quizzes:[...Object.values(BUILTIN_QUIZZES).map(q=>({id:q.id,title:q.title,author:q.author,count:q.questions.length,levels:difficultyCounts(q.questions),language:q.language,builtin:true,download:'/api/quizzes/builtin/'+q.id})),...reviewed.map(q=>({...q,download:'/api/quizzes/content/'+q.sha256,playUrl:'https://play.retro-museum.net/g/'+q.id}))]});return true;
  }
  ...
  if(req.method==='POST'&&path==='/api/quizzes/submissions'){
   try{
    ...
    let size=0;const chunks=[];for await(const chunk of req){size+=chunk.length;if(size>MAX_QUIZ_BYTES){json(res,413,{error:'Quiz exceeds 10 MB.'});return true;}chunks.push(chunk);}
    const input=JSON.parse(Buffer.concat(chunks).toString('utf8')),quiz=validateQuiz(input.quiz),decl=publication(input.publication),hash=sha(JSON.stringify(quiz)),identity=sha(JSON.stringify({quiz,publication:decl})).slice(0,32),key='quiz-dedup/'+identity+'.json';uniqueQuestions(quiz);if(quiz.questions.some(q=>q.difficulty===undefined))throw Error('Every submitted question needs difficulty 1–4.');const existing=await inbox.get(key);
